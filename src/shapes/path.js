...
export const Interface = ({ layer, path, fromMask }) => {
    ...
    return <Wrapper layer={layer} path={path} name='Path' withVisibility>
        <Warning title={"Proceed With Caution"}>
            ...
            <p>This path is dependant coordinate system of the canvas. For your reference, know that the canvas's viewBox is '<code className='highlight selectable'>{`${w.value * w.unit / -2} ${h.value * h.unit / -2} ${w.value * w.unit} ${h.value * h.unit}`}</code>' and all points are drawn in that coordinate space.</p>
        </Warning>
